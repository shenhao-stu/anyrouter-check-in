...
> `anyrouter` 和 `agentrouter` 已内置配置；其它平台统一通过 `PROVIDERS` secret 或插件自动注册。
...
其余平台请通过 `PROVIDERS` secret 或插件自动同步生成。标准 new-api / one-api 站点会使用默认路径（`/api/user/self`、`/api/user/sign_in`，404 时自动回退到 `/api/user/checkin`）。
